import { PrismaClient, Role, EmploymentStatus, JobType } from '@prisma/client'
...
async function main() {
  ...
  const managerUser = await prisma.user.upsert({ where: { email: 'manager@floorstaff.local' }, update: {}, create: { email: 'manager@floorstaff.local', passwordHash, firstName: 'Ana', lastName: 'Manager', role: Role.MANAGER, city: 'Tbilisi', country: 'Georgia' } })
  const employeeUser = await prisma.user.upsert({ where: { email: 'employee@floorstaff.local' }, update: {}, create: { email: 'employee@floorstaff.local', passwordHash, firstName: 'Nino', lastName: 'Beridze', role: Role.EMPLOYEE, city: 'Tbilisi', country: 'Georgia', employeeProfile: { create: { position: 'Head Waiter' } } } })
  await prisma.user.upsert({ where: { email: 'customer@floorstaff.local' }, update: {}, create: { email: 'customer@floorstaff.local', passwordHash, firstName: 'Demo', lastName: 'Customer', role: Role.CUSTOMER, city: 'Tbilisi', country: 'Georgia' } })
  ...
  await prisma.employment.upsert({ where: { employeeId_restaurantId_status: { employeeId: employee.id, restaurantId: restaurant.id, status: EmploymentStatus.ACTIVE } }, update: {}, create: { employeeId: employee.id, restaurantId: restaurant.id, position: 'Head Waiter', status: EmploymentStatus.ACTIVE, startedAt: new Date() } })
  await prisma.job.upsert({ where: { id: 'seed-senior-waiter' }, update: {}, create: { id: 'seed-senior-waiter', restaurantId: restaurant.id, title: 'Senior Waiter', description: 'Lead service with a warm, detail-oriented team.', requirements: 'Two years of guest-facing experience.', salary: '1,800–2,200 GEL', type: JobType.FULL_TIME, city: 'Tbilisi' } })
