...
import { Role, RatingType, EmploymentStatus, JobType } from '@prisma/client'

const app = express()
app.use(cors({ origin: process.env.CLIENT_ORIGIN || 'http://localhost:5173' }))
app.use(express.json({ limit: '1mb' }))

const registerSchema = z.object({ email: z.string().email(), password: z.string().min(8), firstName: z.string().min(1), lastName: z.string().min(1), role: z.nativeEnum(Role).default(Role.CUSTOMER), city: z.string().optional(), country: z.string().optional(), position: z.string().optional() })
const loginSchema = z.object({ email: z.string().email(), password: z.string().min(1) })
const ratingSchema = z.object({ targetEmployeeId: z.string(), restaurantId: z.string(), type: z.nativeEnum(RatingType), score: z.number().min(1).max(5), comment: z.string().max(1000).optional(), teamwork: z.number().min(1).max(5).optional(), reliability: z.number().min(1).max(5).optional(), professionalism: z.number().min(1).max(5).optional(), communication: z.number().min(1).max(5).optional(), workQuality: z.number().min(1).max(5).optional() })
...
app.get('/api/restaurants/:id', async (req, res, next) => {
  try {
    const restaurant = await prisma.restaurant.findUnique({ where: { id: req.params.id }, include: { employments: { where: { status: EmploymentStatus.ACTIVE }, include: { employee: { include: { user: true, badges: { include: { badge: true } } } } } }, jobs: true, reviews: { where: { status: 'APPROVED' }, include: { author: true }, orderBy: { createdAt: 'desc' } }, manager: { include: { user: true } } } })
...
app.post('/api/reviews', requireAuth, requireRole(Role.CUSTOMER), async (req: AuthRequest, res, next) => {
  try {
    const input = reviewSchema.parse(req.body)
    const employee = await prisma.employee.findUnique({ where: { id: input.employeeId }, include: { employments: true } })
    if (!employee || !employee.employments.some((item) => item.restaurantId === input.restaurantId && item.status === EmploymentStatus.ACTIVE)) return res.status(400).json({ error: 'Employee is not active at this restaurant' })
...
app.post('/api/manager-ratings', requireAuth, requireRole(Role.EMPLOYEE), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (!employee || !manager || manager.restaurantId !== input.restaurantId || !employee.employments.some((item) => item.restaurantId === input.restaurantId && item.status === EmploymentStatus.ACTIVE)) return res.status(403).json({ error: 'You must be an active employee at this workplace' })
...
app.get('/api/rankings', async (req, res, next) => {
  try {
    ...
    const ranked = employees.map((employee) => {
      const customer = employee.ratings.filter((rating) => rating.type === RatingType.CUSTOMER)
      const coworkers = employee.ratings.filter((rating) => rating.type === RatingType.COWORKER)
      const raw = (customer.reduce((sum, rating) => sum + rating.score, 0) / (customer.length || 1)) * 0.65 + (coworkers.reduce((sum, rating) => sum + rating.score, 0) / (coworkers.length || 1)) * 0.35
    ...
    }).sort((a, b) => b.rating - a.rating || b.ratingCount - a.ratingCount).map((employee, index) => ({ rank: index + 1, ...employee }))
    res.json(ranked)
  } catch (error) { next(error) }
})
...
app.get('/api/employees/:id', async (req, res, next) => {
  try {
    const employee = await prisma.employee.findUnique({ where: { id: req.params.id }, include: { user: true, badges: { include: { badge: true } }, employments: { where: { status: EmploymentStatus.ACTIVE }, include: { restaurant: true } }, ratings: { where: { moderation: 'APPROVED' }, select: { type: true, score: true, comment: true, createdAt: true } }, reviews: { where: { status: 'APPROVED' }, select: { score: true, comment: true, createdAt: true } } } })
    if (!employee) return res.status(404).json({ error: 'Employee not found' })
    const customerRatings = employee.ratings.filter((rating) => rating.type === RatingType.CUSTOMER)
    const coworkerRatings = employee.ratings.filter((rating) => rating.type === RatingType.COWORKER)
    res.json({ id: employee.id, name: `${employee.user.firstName} ${employee.user.lastName}`, city: employee.user.city, position: employee.position, workplaces: employee.employments.map((item) => ({ id: item.restaurant.id, name: item.restaurant.name, city: item.restaurant.city })), badges: employee.badges.map((item) => item.badge), customerRating: average(customerRatings.map((item) => item.score)), coworkerRating: average(coworkerRatings.map((item) => item.score)), totalRatings: employee.ratings.length, reviews: employee.reviews })
  } catch (error) { next(error) }
})

app.post('/api/employments/request', requireAuth, requireRole(Role.EMPLOYEE), async (req: AuthRequest, res, next) => {
  try {
    const input = z.object({ restaurantId: z.string(), position: z.string().min(2) }).parse(req.body)
    const employee = await prisma.employee.findUnique({ where: { userId: req.user!.id } })
    if (!employee) return res.status(404).json({ error: 'Employee profile not found' })
    const request = await prisma.employment.create({ data: { employeeId: employee.id, restaurantId: input.restaurantId, position: input.position, status: EmploymentStatus.PENDING } })
    res.status(201).json(request)
  } catch (error) { next(error) }
})

app.patch('/api/employments/:id', requireAuth, requireRole(Role.MANAGER), async (req: AuthRequest, res, next) => {
  try {
...
app.post('/api/manager/jobs', requireAuth, requireRole(Role.MANAGER), async (req: AuthRequest, res, next) => {
  try {
    const input = z.object({ title: z.string().min(2), description: z.string().min(10), requirements: z.string().min(2), salary: z.string().optional(), type: z.nativeEnum(JobType), city: z.string().min(2) }).parse(req.body)
    const manager = await prisma.manager.findUnique({ where: { userId: req.user!.id } })
...
app.patch('/api/admin/reviews/:id', requireAuth, requireRole(Role.ADMIN), async (req: AuthRequest, res, next) => {
  try {
...
app.patch('/api/admin/users/:id/suspension', requireAuth, requireRole(Role.ADMIN), async (req: AuthRequest, res, next) => {
  try {
...
app.post('/api/employee/qr', requireAuth, requireRole(Role.EMPLOYEE), async (req: AuthRequest, res, next) => {
  try {
    const employee = await prisma.employee.findUnique({ where: { userId: req.user!.id } })
    if (!employee) return res.status(404).json({ error: 'Employee profile not found' })
